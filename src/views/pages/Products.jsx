...
import { Button, Card, CardHeader, Container, Row, Col, CardFooter, CardBody, Input, Modal, Alert } from "reactstrap";
...
class Products extends React.Component {
	...
	componentDidMount = () => {
		...
		axios({
			method: "get",
			url: "https://kbsgolfx-db.herokuapp.com/Products",
			headers: {
				Authorization: `Bearer ${JSON.parse(localStorage.getItem("golfx_token"))}`
			}
		})
	...
	DeleteClub = () => {
		...
		for (let i = 0; i < listID.length; i++) {
			axios.delete("https://kbsgolfx-db.herokuapp.com/clubs/" + listID[i].id).then((res) => {
				const Products = res.data;
	...
	DeleteProducts = () => {
		...
		for (let i = 0; i < listIDProducts.length; i++) {
			axios.delete("https://kbsgolfx-db.herokuapp.com/products/" + listIDProducts[i].id).then((res) => {
				const Products = res.data;
				console.log(Products);
			});
		}
	...
	newProducts = (e) => {
		...
		if (document.getElementById("input-products").value === "" || this.state.ProductCategory === "") {
			...
			if (document.getElementById("input-products").value) {
				axios({
					...
					data: {
						product_name: document.getElementById("input-products").value.toUpperCase(),
					...
					{(props) => (
						<div className="py-4 table-responsive">
							<div id="datatable-basic_filter" className="dataTables_filter px-4 pb-1">
								<i className="far fa-trash-alt text-primary mr-3" onClick={(e) => this.DeleteProducts()}></i>
								<label>
									Search:
							...
							<BootstrapTable {...props.baseProps} bootstrap4={true} pagination={pagination} bordered={false} selectRow={selectRowProducts} />
						</div>
					)}
				</ToolkitProvider>
			);
	...
	render_modal_Prodcuts = () => {
		return (
			<Modal className="modal-dialog-centered" isOpen={this.state.notificationModal} toggle={() => this.toggleModal("notificationModal")}>
				...
				<div className="container">
					<Alert color="warning" isOpen={this.state.productError} fade={true}>
						...
						<span className="alert-inner--text">
							<strong>Error!</strong> Please fill both Inputs!{this.state.errorMessage}
						</span>
					</Alert>
				</div>

				<div className="modal-footer">
					<Button color="primary" type="button" onClick={(e) => this.newProducts(e)}>
						Add product
					</Button>
					<Button className="ml-auto" color="link" data-dismiss="modal" type="button" onClick={() => this.toggleModal("notificationModal")}>
						Close
					</Button>
				</div>
			</Modal>
	...
	render_modal_Oem = () => {
		return (
			<Modal className="modal-dialog-centered" isOpen={this.state.oemModal} toggle={() => this.toggleModal("oemModal")}>
				<div className="modal-header">
					<h6 className="modal-title" id="modal-title-default">
						Add New Oem
					</h6>
					<button aria-label="Close" className="close" data-dismiss="modal" type="button" onClick={() => this.toggleModal("oemModal")}>
						<span aria-hidden={true}>×</span>
					</button>
				</div>
				<div className="modal-body">
				...
				<div className="modal-footer">
					...
					<Button className="ml-auto" color="link" data-dismiss="modal" type="button" onClick={() => this.toggleModal("oemModal")}>
						Close
					</Button>
				</div>
			</Modal>
	...
	render_modal_Club = () => {
		return (
			<Modal className="modal-dialog-centered" isOpen={this.state.ClubModal} toggle={() => this.toggleModal("ClubModal")}>
				<div className="modal-header">
					<h6 className="modal-title" id="modal-title-default">
						Add New Club
					</h6>
					<button aria-label="Close" className="close" data-dismiss="modal" type="button" onClick={() => this.toggleModal("ClubModal")}>
						<span aria-hidden={true}>×</span>
					</button>
				</div>
				<div className="modal-body">
				...
				<div className="modal-footer">
					<Button color="primary" type="button" onClick={(e) => this.newClub(e)}>
						Add Club
					</Button>
					<Button className="ml-auto" color="link" data-dismiss="modal" type="button" onClick={() => this.toggleModal("ClubModal")}>
						Close
					</Button>
				</div>
			</Modal>
			...
			<>
				...
				<Container className="mt--6" fluid>
					<Row>
						...
						<div className="col">{this.renderTable("Shafts", "Add or Search Shafts", "Add Shafts", "notificationModal")}</div>
					</Row>
				</Container>
